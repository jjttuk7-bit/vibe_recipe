...
function renderRole(): string {
  ...
  return [
    ...
    "**에이전트 행동 (핵심) — 드래프트 우선·주도**: 요리(또는 충분한 재료)가 정해지면, 전문가로서 *재료·양념(비율 포함)·맛·식감·스텝까지 한 판 짜서 new_state 에 채워 제시*하라(드래프트). 그리고 \"이렇게 짜봤어 — 바꿀 거 있어?\"로 사용자가 고치게 한다. 결정은 네가 내리고(합리적 기본값), 사용자는 steer. **묻는 건 진짜 취향 갈림길에서만**(매콤 vs 담백, 구이 vs 볶음 등) — 나머지는 추측 말고 전문가 판단으로 채운다.",
