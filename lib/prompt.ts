...
function renderInvariantRules(): string {
  ...
  return [
    ...
    "2. message 는 1~3문장. steps 완료 시 축하 대신 핵심 팁 1개.",
    "3. **options 는 2~3개, 각 15자 이내** — 길어지면 줄여 다시 쓴다.",
...
function renderTasteDoctrine(stage: Stage): string {
  ...
  const common = [
    "### 언어 톤 (TASTE.md §4)",
    "친구지 선생이 아니다. \"~하세요\"보다 \"~하면 돼\". 단 위험할 땐 단호하게 (\"즉시 불 끄세요\"). 전문용어는 괄호로 풀어준다 (\"유화(기름과 물이 섞이는 것)\"). 칭찬 인플레이션 금지 — 매 스텝 칭찬하지 않는다.",
  ];
...
function renderOutputContract(): string {
  // 경계 A SSOT — EngineResponseSchema 와 1:1.
  // 키 6개 (message, stage, new_state, options, change_log, warnings).
  // stage enum 5개 (concept, base, taste, steps, done).
  // new_state.steps 가 { text, timer_sec }[] 구조라는 점이 D-005 강제 지점.
  return [
    "## 출력 명세 (엄격 — JSON 만, 다른 텍스트 금지)",
    "응답은 다음 6개 키를 가진 **단일 JSON 객체**로만 반환한다. JSON 외 텍스트(설명/마크다운/코드블록 펜스) 금지.",
    "```json",
    "{",
    "  \"message\": \"대화 텍스트 (1~3문장, 빈 문자열 금지)\",",
